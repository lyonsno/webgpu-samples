...
@group(0) @binding(3) var<storage, read_write> errors: array<i32>;
...
fn main(@builtin(global_invocation_id) id: vec3u) {
  ...
  for (var row = 0u; row < patchWidth; row++) {
    for (var col = 0u; col < patchWidth / 4; col++) {
      ...
      let t = target[row * (patchWidth / 4) + col];
      if (packed) {
        // Sum (p-t)^2 = sum p^2 + sum t^2 - 2 sum p*t.
        // The template's self-dot is shared by every candidate.
        error += dot4I8Packed(p, p) - 2 * dot4I8Packed(p, t);
      } else {
  ...
  errors[candidate] = error + select(0, templateEnergy, packed);
}
